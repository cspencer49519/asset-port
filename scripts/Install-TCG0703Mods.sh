#!/usr/bin/env bash
# Installs TCGShopExpansionMod0703Patch (and optional ported sharedassets) into a 0.70.3 game folder.
set -euo pipefail
...
usage() {
    cat <<'EOF'
Usage: Install-TCG0703Mods.sh [--game-path PATH] [--skip-assets] [--force] [--dry-run]

Installs TCGShopExpansionMod0703Patch and optional sharedassets into a 0.70.3 game folder.
EOF
}
...
resolve_patch_dll() {
    local release_dll="${RELEASE_ROOT}/patches/TCGShopExpansionMod0703Patch.dll"
    if [[ -f "${release_dll}" ]]; then
        printf '%s\n' "${release_dll}"
        return 0
    fi

    local dev_dll="${RELEASE_ROOT}/TCGShopExpansionMod0703Patch/bin/Release/netstandard2.1/TCGShopExpansionMod0703Patch.dll"
    if [[ -f "${dev_dll}" ]]; then
...
PLUGIN_DIR="${GAME_ROOT}/BepInEx/plugins/TCGShopExpansionMod0703Patch"
PLUGIN_DLL="${PLUGIN_DIR}/TCGShopExpansionMod0703Patch.dll"
...
echo "  1. Install Nexus mods + Genobear (phases 1-3 in docs/INSTALL-0703.md) if not done yet"
echo "  2. Run: ./scripts/Verify-TCG0703Install.sh --game-path \"${GAME_ROOT}\""
