...
.EXAMPLE
  .\Install-TCG0703Mods.ps1 -GamePath "D:\Steam\steamapps\common\TCG Card Shop Simulator"
#>
...
function Resolve-PatchDll([string] $ReleaseRoot) {
    $releaseDll = Join-Path $ReleaseRoot "patches\TCGShopExpansionMod0703Patch.dll"
    if (Test-Path $releaseDll) {
        return (Resolve-Path -LiteralPath $releaseDll).Path
    }

    $devDll = Join-Path $ReleaseRoot "TCGShopExpansionMod0703Patch\bin\Release\netstandard2.1\TCGShopExpansionMod0703Patch.dll"
    if (Test-Path $devDll) {
...
$pluginDir = Join-Path $gameRoot "BepInEx\plugins\TCGShopExpansionMod0703Patch"
$pluginDll = Join-Path $pluginDir "TCGShopExpansionMod0703Patch.dll"
...
Write-Host "  1. Install Nexus mods + Genobear (phases 1-3 in docs/INSTALL-0703.md) if not done yet"
Write-Host "  2. Run: .\scripts\Verify-TCG0703Install.ps1 -GamePath `"$gameRoot`""
