...
internal sealed class PackOpeningLateSyncBehaviour : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (!_loggedBootstrapGiveUp && _bootstrapRetryCount >= BootstrapGiveUpRetries)
        {
            ...
            Plugin.Log.LogWarning(
                ...
                "Pack wrapper animation may stay broken until scene objects are restored from vanilla 0.70.3.");
        }
    }
    ...
    private void LateUpdate()
    {
        ...
        try
        {
            ...
            if (state is >= 0 and < 7)
            {
                PackOpening0703Patches.SyncAllPackOpeningPresentations(sequence);
            }
            else if (state >= 7)
            {
                PackOpening0703Patches.HidePackVisualsDuringFanRow(sequence);
                PackOpening0703Patches.SyncAllFanRowPresentations(sequence);
            }
        }
